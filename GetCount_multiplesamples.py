...
def write_matrices(counts_df, nf_df, out_dir, out_prefix):
    ...
    (out_dir / f"{out_prefix}_rawcounts.txt").write_text(
        counts_out.to_csv(sep="\t", index=False, header=True)
    )  # write_text to ensure atomic-ish write; to_csv returns str when not given path

    nf_out = nf_df.copy()
    nf_out.insert(0, "loop_id", nf_out.index)
    (out_dir / f"{out_prefix}_normalizationFactor.txt").write_text(
...
def GetCount(inputpath, filename, loopfile, outpath, outfile):
    ...
    man = read_manifest(str(Path(inputpath) / filename))
    counts_df, nf_df = build_matrices(man, loops_all, meta)
    write_matrices(counts_df, nf_df, outpath, outfile)
...
def main():
    ...
    GetCount(args.inputpath, args.input, args.loopfile, args.outpath, args.outfile)
